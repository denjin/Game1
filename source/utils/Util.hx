...
class Util
{
	...
	public static function rotate(target:FlxPoint, pivot:FlxPoint, angle:Float):FlxPoint
	{
		var radians:Float = degToRad(angle);
		var sin:Float = FlxMath.fastSin(radians);
		var cos:Float = FlxMath.fastCos(radians);
		
		var dx:Float = target.x - pivot.x;
		var dy:Float = target.y - pivot.y;
		var output:FlxPoint = new FlxPoint(cos * dx - sin * dy + pivot.x, sin * dx + cos * dy + pivot.y);
		return output;
	}
	...
	public static function worldPointToScreenPoint(point:FlxPoint):FlxPoint
	{
		return new FlxPoint(point.x - Global.instance.screen.x, point.y - Global.instance.screen.y);
	}
	
	public static function screenPointToWorldPoint(point:FlxPoint):FlxPoint
	{
		return new FlxPoint(point.x + Global.instance.screen.x, point.y + Global.instance.screen.y);
	}
	
	public static function getHyp(x:Float, y:Float):Float
	{
